...
iicInit(void)
{
    ...
    lmx2594Values[2] = lmx2594Defaults;
    lmx2594Sizes[0] = LMX2594_SIZE;
    lmx2594Sizes[1] = LMX2594_SIZE;
    lmx2594Sizes[2] = LMX2594_SIZE;
     ...
     * around a design error on the BCM111 that resulted in the fan always
...
lmk04208read(int reg)
{
    ...
    buf[1] = (reg & 0x1F);
    buf[2] = 0;
    buf[3] = 31;  /* Don't bother with register lock */
    if (!spiSend(SPI_MUX_04208, buf, 4)) return 0;

    /*
     * Read back value
     * Transmit values are ignored, so just send the same thing.
     */
    if (!spiTransfer(SPI_MUX_04208, buf, 4)) return 0;
    return (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];
}

int
lmk04208write(uint32_t value)
{
    uint8_t buf[4];
